...
export class WijitTester extends HTMLElement {
		...
		this.shadowRoot.innerHTML = `
			<style>
				...
				/****** Borders ******/
					...
					output details:last-child summary {
			...
			<article>
				...
				<section id="progress-container" class="sticky row">

					<button id="pause" class="no-show">||</button>
	...
	connectedCallback () {
		...
		this.addEventListener ('testfailed', event => {
			if (this.stopOnError) this.pause();
	...
	getTests (component) {
		...
		for (const match of matches) {
			...
			item.add ([Function ('self', func), expected]);
			i++;
		}
	...
	setup () {
		...
		start.disabled = true;
		pause.classList.remove('no-show');
	}

	initTester (tests, tag) {
		let i = 1, interval;
		const output = this.shadowRoot.querySelector('output');
		output.innerHTML = '';

		if (window.Tester) {
			const tester = new Tester (tests, tag, this);
			...
			interval = setInterval (() => {
				...
				if (window.Tester) {
					...
					clearInterval (interval);
					console.error ('Could not instantiate Tester');
				}
			}, 10);
		}
	}
	...
	async startTests (tests, tester) {
		for (const [key, items] of tests) {
			for (const [func, predicted] of items) {
				...
				tester.doTest (func, predicted, key);
			}
		}
	}
	...
	addResult (event) {
		...
		if (this.onlyErrors && verdict === 'pass') {
			...
			const description = data[4].replace(/{return|}$/g, '');
			const expected = `<b>Expected:</b> ${data[3][1]} (${typeof data[3][1]})`;
			const result = `<b>Result:</b> ${data[2][1]} (${typeof data[2][1]})`;
			summary.classList.add(verdict);
			summary.innerHTML = `${idx} <b>${verdict}:</b> ${description}`;
			detail.innerHTML = `<code>\t${result} | ${expected}</code>`;
...
export class Tester {
	...
	doTest(func, predicted, key) {
		this.init (this.instance.constructor);
		const self = this.instance;
		// let desc = func.toString().replace(/\n/g, '').match (/{.*}/)[0];
		let desc = func.toString().replace(/^[^{]+\{\s*return|\n|\t|}$/g, '');
		desc += ` // ${predicted}`;
		...
		.then (result => {
			...
			if (verdict === 'fail') {
				...
				this.caller.dispatchEvent(failEvent);
			}
			const ret = [
				this.testNum,
				verdict,
